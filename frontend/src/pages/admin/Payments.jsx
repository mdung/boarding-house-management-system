        ...
        <div className="flex items-center justify-between mb-3 flex-wrap gap-3">
          ...
          <div className="relative">
            ...
            <input value={paySearch} onChange={e => { setPaySearch(e.target.value); setPayPage(1) }} placeholder="Search payments..."
              className="pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-2xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all w-64" />
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          {totalPages > 1 && (
            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/40 flex items-center justify-between">
              <p className="text-xs font-bold text-slate-400">{(payPage-1)*PAGE_SIZE+1}–{Math.min(payPage*PAGE_SIZE, filtered.length)} of {filtered.length}</p>
              <div className="flex items-center gap-1">
                <button disabled={payPage===1} onClick={() => setPayPage(p=>p-1)} className="w-8 h-8 flex items-center justify-center bg-white border border-slate-200 rounded-xl disabled:opacity-40 hover:bg-slate-50 shadow-sm transition-all"><ChevronUp className="w-4 h-4 rotate-90" /></button>
                {Array.from({length: Math.min(5, totalPages)}, (_,i) => {
                  const p = Math.max(0, Math.min(payPage-3, totalPages-5)) + i + 1
                  return <button key={p} onClick={() => setPayPage(p)} className={`w-8 h-8 rounded-xl text-sm font-bold transition-all ${p===payPage ? 'bg-slate-900 text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>{p}</button>
                })}
                <button disabled={payPage===totalPages} onClick={() => setPayPage(p=>p+1)} className="w-8 h-8 flex items-center justify-center bg-white border border-slate-200 rounded-xl disabled:opacity-40 hover:bg-slate-50 shadow-sm transition-all"><ChevronDown className="w-4 h-4 rotate-90" /></button>
              </div>
            </div>
          )}
        </div>
